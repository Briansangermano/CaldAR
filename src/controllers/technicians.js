...
exports.update = (req, res) => {
  ...
  Technicians.findOneAndUpdate(
    { id_technicians: req.body.id_technicians },
    req.body,
    { useFindAndModify: false }
  )
    .then(() => res.send({ message: `Technician was updated` }))
    .catch((err) => {
      res.status(500).send({
        message:
...
exports.delete = (req, res) => {
  ...
  Technicians.findOneAndRemove({ id }, { useFindAndModify: false })
    .then(() => res.send({ message: "Technician was remove successfully" }))
    .catch(() => {
      res.status(500).send({
